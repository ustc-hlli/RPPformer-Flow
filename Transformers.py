...
class RPPAttenLayer(nn.Module):
    ...
    def forward(self, xyz1, xyz2, feat1, feat2):
        ...
        early_a_code_1 = self.early_q_encoder(real_xyz) #[B, C, N1, S]
        early_a_code_2 = self.early_k_encoder(real_xyz) 
        ...
        a1 = self.qk_encoder(feat1_real.mul(early_a_code_1)) #[B, C, N1, S] 
        a2 = feat2_real.mul(early_a_code_2) 
        v = self.v_encoder(feat2_real.mul(early_v_code_1)).mul(early_v_code_2) #[B, C, N1, S]


        atten = torch.einsum('bcns, bcns -> bns', a1, a2) #[B, N1, S]
        atten = torch.softmax(atten/np.sqrt(a2.size()[1]), dim=-1)
